...
describe 'fetch_config:run' do
  ...
  context "when running a HeadNode from kitchen on update" do
      cached(:cluster_shared_dir) { '/cluster_shared_dir' }
      cached(:cluster_shared_dir_login_nodes) { '/shared_dir_login_nodes' }
      cached(:cluster_config_path) { 'cluster_config_path' }
      cached(:login_cluster_config_path) { 'login_cluster_config_path' }
      cached(:previous_cluster_config_path) { 'cluster_config_path' }
      cached(:cluster_config_version) { 'cluster_config_version' }
      ...
      cached(:instance_types_data_path) { 'instance_types_data_path' }
      cached(:previous_instance_types_data_path) { 'previous_instance_types_data_path' }
      cached(:change_set_path) { 'change_set_path' }
      cached(:shared_login_nodes_keys_sync_file_path) { '/shared_dir_login_nodes/.login_nodes_keys_sync_file' }
      cached(:chef_run) do
        runner = ChefSpec::Runner.new(
          platform: 'ubuntu', step_into: %w(fetch_config)
        ) do |node|
          ...
          node.override['cluster']['instance_types_data_path'] = instance_types_data_path
          node.override['cluster']['previous_instance_types_data_path'] = previous_instance_types_data_path
          node.override['cluster']['change_set_path'] = change_set_path
          ...
          node.override['cluster']['shared_dir_login_nodes'] = cluster_shared_dir_login_nodes
          node.override['cluster']['shared_login_nodes_keys_sync_file'] = shared_login_nodes_keys_sync_file_path
          node.override['cluster']['node_type'] = 'HeadNode'
        end
        allow(File).to receive(:exist?).with(cluster_config_path).and_return(true)
        allow(FileUtils).to receive(:cp_r).with(
          cluster_config_path, previous_cluster_config_path, remove_destination: true
        ).and_return(true)
        allow(File).to receive(:read).with(cluster_config_path).and_return('FAKE_CONFIG_CONTENT')
        allow(FileUtils).to receive(:identical?).with(
          cluster_config_path, previous_cluster_config_path
        ).and_return(true)
        allow(FileUtils).to receive(:cp_r).with(
          instance_types_data_path, previous_instance_types_data_path, remove_destination: true
        ).and_return(true)
        allow(File).to receive(:read).with(change_set_path).and_return('FAKE_CHANGE_SET_CONTENT')
        allow(FileUtils).to receive(:cp_r).with(
      ...
      it "writes the synchronization file for login nodes" do
        is_expected.to create_file(shared_login_nodes_keys_sync_file_path).with(
          content: config_version_file,
          mode: '0644',
          owner: 'root',
          group: 'root'
        )
