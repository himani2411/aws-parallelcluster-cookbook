...
describe 'nvidia_imex:nvidia_kernel_module?' do
  [%w(false), [false], %w(no), %w(true), [true], %w(yes)].each do |force_indicator|
    context "node['cluster']['nvidia']['imex']['force_configuration'] is #{force_indicator}" do
      cached(:chef_run) do
      ...
      it "is #{force_indicator}" do
        allow_any_instance_of(Object).to receive(:nvidia_kernel_module?).and_return(force_indicator)
        expect(resource.nvidia_kernel_module?).to eq(force_indicator)
      end
    end
  end
end
          ...
          context "when get_nvswitch_count > 1 on #{node_type} node" do
            cached(:chef_run) do
              stubs_for_provider('nvidia_imex[configure]') do |pro|
                ...
                allow(pro).to receive(:nvidia_kernel_module?).and_return(force_indicator)
              end
              runner(platform: platform, version: version, step_into: ['nvidia_imex'])
            end
            cached(:node) { chef_run.node }
